    ...
    try {
        ...
        const upsertPromises = response.data.results.map((item) => {
            const itemData = {
                ...
                release_date:
                    type === 'movie' ? (item.release_date ? new Date(item.release_date) : null) : null,
                first_air_date:
                    type === 'tv' ? (item.first_air_date ? new Date(item.first_air_date) : null) : null,
                genres: [],
...
// Generic function for videos/trailers
export async function getVideos(type, id) {
    try {
        ...
        console.error(
            `Error fetching videos for ${type} ${id}:`,
        ...
        return {
            success: false,
            error: `Failed to fetch videos for ${type} ${id}`,
            status: error.response?.status || 500,
        };
    }
}
...
export async function getRecommendations(type, id, page = 1) {
    try {
        const endpoint =
            type === 'movie' ? `/movie/${id}/recommendations` : `/tv/${id}/recommendations`;
        const response = await tmdbApi.get(endpoint, {
            params: { page },
        });
